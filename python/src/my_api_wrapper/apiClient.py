...
class MeteoritesAPIClientError(Exception):
...
class ValidationError(MeteoritesAPIClientError):
...
class MeteoritesAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"name": {"type": "string", "required": False}, "mass": {"type": "number", "required": False}, "year": {"type": "number", "required": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the MeteoritesAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises MeteoritesAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/meteorites'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise MeteoritesAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[MeteoritesAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        :raises MeteoritesAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise MeteoritesAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise MeteoritesAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise MeteoritesAPIClientError(f"Request failed: {str(e)}")
